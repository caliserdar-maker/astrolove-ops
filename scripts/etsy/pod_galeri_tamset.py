...
def main():
    ...
    for sira, c in enumerate(secim, 1):
        ...
        A_, B_ = burclar(c)
        foto = [(g["sira"], d / g["dosya"], g["cl_karsiligi"]) for g in SET["galeri"]]
        ...
        with open(vpath, "rb") as fh:
            rv = api.post_file(f"/shops/{shop}/listings/{lid}/videos", files={"video": (f"{c}.mp4", fh, "video/mp4")},
                               data={"name": f"{c}.mp4"})
        # geri okuma
        g2 = kararli(lambda: [(x.get("listing_image_id"), x.get("rank"), x.get("alt_text")) for x in galeri(api, lid)],
                     lambda g: len(g) == 13)
        ...
        beklenen = [(yeni[n], n) for n in sorted(yeni)]
        kontrol = {
            "foto_13": len(g2 or []) == 13,
            "sira": [(x[0], x[1]) for x in g2 or []] == beklenen,
            "alt_metin": all((x[2] or "") == alt_uyarla(ref_alt.get(foto[x[1] - 1][2], ""), A_, B_)[:250] for x in g2 or []),
            "video_1": len(v2 or []) == 1,
