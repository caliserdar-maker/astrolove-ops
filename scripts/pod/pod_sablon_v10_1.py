#!/usr/bin/env python3
"""POD sablon V10.1 - V10 + yalniz iki duzeltme.
...
def main():
    ...
    for ad in OGELER:
        ...
        # V10.1/B: yalniz Y ekseninde -- RGB'ye ayni sabit eklenir, BT.601
        # U/V katsayilari toplami 0 oldugu icin renk (U/V) degismez.
        l_r = luma(renk)
        koyu = cek & (l_r < 42.0)
        if koyu.any():
            renk[koyu] += (42.0 - l_r[koyu])[:, None]
    ...
    video = out / "Aquarius_Aries_4570110641_video_V10_1.mp4"
    yuv_video_yaz(ham_yuv, video, vw, vh, fps, sure)
    kapak = out / "Aquarius_Aries_4570110641_kapak_V10_1.png"
    ...
    ocr_gecti = (all(b in ocr_yeni.split() for b in beklenen)
                 and harf_yeni == sum(len(b) for b in beklenen))
    ...
    # V10.1 U/V kapisi (ana sembol halka bandi, kare 0, kodlanmis video)
    ru, rv = panel_uv(ref_veri, kare_bayt, 0, kutu, vw, vh)
    nu, nv = panel_uv(yeni_veri, kare_bayt, 0, kutu, vw, vh)
    rb, nb = halka_bandi(ref_ana), halka_bandi(ana_m)
    uv = {"referans_U": round(float(ru[rb].mean()), 2),
          "referans_V": round(float(rv[rb].mean()), 2),
          "yeni_U": round(float(nu[nb].mean()), 2),
          "yeni_V": round(float(nv[nb].mean()), 2),
          "tum_maske": {"referans_U": round(float(ru[ref_ana].mean()), 2),
                        "referans_V": round(float(rv[ref_ana].mean()), 2),
                        "yeni_U": round(float(nu[ana_m].mean()), 2),
                        "yeni_V": round(float(nv[ana_m].mean()), 2)}}
    uv["gecti"] = bool(abs(uv["referans_U"] - uv["yeni_U"]) <= 2.0
                       and abs(uv["referans_V"] - uv["yeni_V"]) <= 2.0)
    uv["tolerans"] = 2.0
    kapilar["UV"] = uv
    ...
    if a.onceki:
        ...
        tol = {"K1": 0.5, "K2": 0, "K3": 0.02, "K4": 0.3, "K5_dis": 0.05,
               "K5_kapak": 0.2}
        sapma["tolerans"] = tol
        farklar = {}
        ...
        for ad, v in kalinlik.items():
            o = onc["K4"]["kalinlik"][ad]["yeni"]
            farklar[f"K4_{ad}"] = [o, v["yeni"], abs(o - v["yeni"]) <= tol["K4"]]
    ...
    log(f"KAPILAR: K1 {kapilar['K1']['gecti']} K2 {kapilar['K2']['gecti']} "
        ...
        f"UV {kapilar['UV']['gecti']} SAPMA {kapilar['SAPMA']['gecti']} -> {hepsi}")
    log(f"OCR ref '{ocr_ref}' yeni '{ocr_yeni}' | UV {uv}")

    # ---- gorseller
    zamanlar = [round(sure * i / 7, 3) for i in range(8)]
    satir = []
    for ad, yol in [("REFERANS  Aquarius + Gemini", ref_video),
                    ("YENI V10.1  Aquarius + Aries", video)]:
        ...
        satir.append((ad, g))
    karsilastirma(satir, out / "KARE_KARSILASTIRMA_V10_1.jpg")
    for i, z in enumerate(zamanlar):
        zz = min(z, max(sure - 1.0 / fps, 0))
        cift = []
