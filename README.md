...
| `icc-softproof` | elle (`pair`, `size`, `profile`, `max_px`, `gamut_esik`) | Baski dosyalarini Prodigi HPR ICC profiliyle soft-proof eder (relative colorimetric + BPC); deltaE00 ort/maks/p95 + gamut disi %, karsilastirma PNG'leri -> Drive `TEMP/ICC_TEST/` |
| `brand-banner` | elle (`line2`, `out_name`, `base_name`, `upload`) | Etsy magaza banner: onayli V7 tabaninda YALNIZ alt satir yeniden cizilir (`scripts/brand/banner_v8.py`), cikti Drive `BRAND/` + artifact; Etsy yuklemesi elle |
...
| `pod-listing-create` | elle (`pairs` / `ALL`, `limit`, `dry_run` varsayilan acik, `primary`, `return_policy_id`, `return_policy_spec`) | POD YENI ILAN: createDraftListing + 12 gorsel (10 kare + Symbol Story/Crafted Detail teknik karti rank 4-5, kart OCR on kontrolu) + V01 video (ana edisyon; eksik medya raporlanir, kosu surer) + Color(5)xSize(16)=80 varyant (SKU `POD-<burc3>_<burc3>-<ed2>-<SIZE>` (<=32), adet 999, fiyat `scripts/etsy/pod_prices.csv`) + renk secenegine edisyon 01 karesi; kargo profili/bolum/partner/iade/taxonomy API'den. dry_run: kesif + payload JSON. STATE `TEMP/POD_LISTING/POD_LISTINGS_STATE.csv` (resume); kota 400 altinda yeni ilana baslamaz |
...
| `pod-inventory-update` | elle (`listing_id`, `pair`, `expected_images`, `dry_run` varsayilan acik, `quota_min`) | POD ilan duzenlemesi: Size etiketleri v3 + oran grubu sirasi (tek updateListingInventory; SKU/fiyat/adet/gorunurluk korunur, geri okuma 80/80 + sira), aciklama yalniz "✦ 16 SIZES" blogu (EN PATCH + RU PUT; baska fark varsa yazma yok), renk->gorsel dogrulama (fark -> POST). Metin `docs/POD_LISTING_TEMPLATE.md` + `SIZE_SPEC` |
| `pod-desc-update` | elle (`limit`, `dry_run` varsayilan acik, `section`, `section_ru`, `quota_min`) | Mevcut POD ilanlarinin aciklamasini sablona esitler: bolum bolum karsilastirma, yalniz secilen bolum degisebilir (aksi halde ilan SIRA DISI, yazma yok), EN `updateListing` + RU `translations/ru` PUT, geri okuma cevaptan; kota tabaninda durur, kalanlari raporlar |
| `pod-night` | cron `10 0 * * *` (TEK SEFERLIK) + elle (`dry_run`, `publish_limit`, `desc_limit`, `quota_min`) | GECE: ADIM 1 "Sold unframed" satirini 78 ilandan geri alir (`pod_desc_update`, bolum ✦ THE ARTWORK / ✦ АРТ); ADIM 1 tamamen PASS ise ADIM 2 alfabetik ilk 5 taslagi yayina alir (`pod_publish`; on kontrol 12 gorsel + 1 video + 65 varyant + bolum/kargo/iade, eksikse yayinlamaz). Cikti `TEMP/POD_LISTING/NIGHT_<MODE>_<stamp>/` |
