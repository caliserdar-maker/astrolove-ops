...
| `pod-inventory-update` | elle (`listing_id`, `pair`, `expected_images`, `dry_run` varsayilan acik, `quota_min`) | POD ilan duzenlemesi: Size etiketleri v3 + oran grubu sirasi (tek updateListingInventory; SKU/fiyat/adet/gorunurluk korunur, geri okuma 65/65 + sira), aciklama yalniz "✦ 13 SIZES" blogu (EN PATCH + RU PUT; baska fark varsa yazma yok), renk->gorsel dogrulama (fark -> POST). Metin `docs/POD_LISTING_TEMPLATE.md` + `SIZE_SPEC` |
